...
- [x] **M7 — Counselor review overhaul** (user request: narrow/collapsible sidebar, AG-Grid queue, OneRoster verify detail)
  - [x] DATA CONTRACT (orchestrator-authored, frozen first per advisor): `mockData.js` gained `ONE_ROSTER` (studentId → authoritative SIS record: gpa, attendanceRate, gradeLevel, enrollmentStatus, completedCourses[], currentSchedule[], lastSync) for all 12 students, and `REVIEW_CHECKS` (requestId → `[{id,label,claimed,actual,passed,reasoning}]`). Checks map to active RUBRIC_CRITERIA ids. **Canned data = the OUTPUT of `evaluateAgainstRubric` — algorithm seam in schedulingLogic.js stays an intentional stub (boundary respected).** `api.js`: `fetchReviewQueue` joins `checks` at the read seam; new `fetchOneRosterRecord(studentId)`.
  - [x] `AppShell.jsx` (sonnet): sidebar `w-60`→`w-52`, collapse toggle (inline SVG, no new dep) → `w-16` icon-rail, localStorage `ccc:sidebar-collapsed` persisted, per-item inline SVG icons + tooltips when collapsed. Header/role-switcher/routing untouched. Pattern adapted from `C:\My projects Main\Brenda Arjun\Brenda\src\components\app-shell\sidebar.tsx`.
  - [x] `ReviewQueue.jsx` (sonnet): rewritten as container — **real AG Grid** (`ag-grid-react`+`ag-grid-community` v35.3.1, React-19 ok) sortable spreadsheet (8 cols, default sort Submitted desc, brand-tuned `themeQuartz.withParams` accent #2456e6, no CSS import — v35 Theming API). Row-click → `fetchOneRosterRecord` + render `<ReviewDetail/>`; `handleDecision` carries the admit/deny + queue-removal flow.
  - [x] `ReviewDetail.jsx` (sonnet, NEW): pure presentational split view. LEFT "Submitted by student" (waiver type, note, docs, requested courses). RIGHT "OneRoster record" (authoritative SIS facts). "Rubric verification" — each check a ✓/✗ `<button aria-expanded>` → click expands Claimed-vs-Actual + reasoning. Carried-over note input + giant Admit/Deny. Prop seam: `{request, waiverName, oneRoster, loadingRoster, submitting, onBack, onDecision}`.
  - [x] `npm run build` green (1.66 MB bundle — AG Grid is heavy; code-split deferred). **Live browser QA (gstack-browse)**: queue grid renders w/ pills+sort, row-click→detail, ✓-expand shows Claimed/Actual/reasoning, sidebar collapse persists, fail case (Liam Park 0/3 red ✗) verified. Screenshots in TEMP.
  - [~] FOLLOW-UPS (deferred, non-blocking): (1) AG Grid 1.6 MB → lazy-load/code-split the admin route; (2) checks currently a subset per student — could render every active criterion uniformly; (3) reconcile note: user picked "Real AG Grid" but Brenda Arjun had no AG Grid, so only the sidebar pattern came from there — grid wired fresh.
